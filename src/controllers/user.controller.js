...
const getUser = catchAsync(async (req, res) => {
  ...
  if(data==null){ throw new ApiError(400,"Invalid Mongo ID"); }
  else if(req.user && id!=req.user._id){ throw new ApiError(403,"User not found"); }
  else if(req.query.q == "address") { var data = await userService.getUserAddressById(id);res.status(200).send({address:data.address}); }
